...
// Initialize protection system (uses quantum_state_t from quantum_types.h)
ProtectionSystem* init_protection_system(const quantum_state_t* state,
                                        const struct IBMConfig* config);
...
bool detect_topological_errors(const quantum_state_t* state,
                              const struct IBMConfig* config);
bool correct_topological_errors(quantum_state_t* state,
                               const struct IBMConfig* config);
bool verify_topological_state(const quantum_state_t* state,
                             const struct IBMConfig* config);

// Error logging and recovery
void log_correction_failure(const quantum_state_t* state,
                           const char* message);

// Enhanced error detection
AnyonSet* detect_mitigated_anyons(const quantum_state_t* state,
                                  const void* config);
...
bool apply_mitigated_correction(quantum_state_t* state,
