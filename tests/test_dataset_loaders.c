...
static void test_mnist_loading() {
    ...
    // Verify data ranges after normalization - use ComplexFloat access
    for (size_t i = 0; i < dataset->num_samples; i++) {
        // Check features (ComplexFloat - access .real component)
        for (size_t j = 0; j < dataset->feature_dim; j++) {
            assert(dataset->features[i][j].real >= 0.0f);
            assert(dataset->features[i][j].real <= 1.0f);
        }

        // Check labels (ComplexFloat - access .real component)
        assert(dataset->labels[i].real >= 0.0f);
        assert(dataset->labels[i].real <= 9.0f);
    }
...
static void test_uci_loading() {
    ...
    for (size_t i = 0; i < dataset->num_samples; i++) {
        // Check features (ComplexFloat - access .real component)
        for (size_t j = 0; j < dataset->feature_dim; j++) {
            assert(dataset->features[i][j].real >= -4.0f);  // Allow some margin
            assert(dataset->features[i][j].real <= 4.0f);   // Allow some margin
        }

        // Check labels (0-2 for Iris classes)
        assert(dataset->labels[i].real >= 0.0f);
        assert(dataset->labels[i].real <= 2.0f);
    }
...
static void test_dataset_splitting() {
    ...
    // Verify class distribution if stratified (using ComplexFloat access)
    if (dataset->num_classes > 0) {
        ...
        for (size_t i = 0; i < split.train_data->num_samples; i++) {
            if (split.train_data->labels[i].real == 0) train_class_ratio += 1.0f;
        }
        for (size_t i = 0; i < split.val_data->num_samples; i++) {
            if (split.val_data->labels[i].real == 0) val_class_ratio += 1.0f;
        }
        for (size_t i = 0; i < split.test_data->num_samples; i++) {
            if (split.test_data->labels[i].real == 0) test_class_ratio += 1.0f;
        }
